...
class Executor:
    ...
    def run(self, plan: Plan, question: str, *, continue_on_error: bool = False) -> Dict[str, Any]:
        ctx: Dict[str, Any] = {}
        step_logs = []
        ...
        for step in plan.steps:
            ...
            try:
                ...
                ctx[step.id] = out
                step_logs.append({"step_id": step.id, "tool": step.tool, "elapsed_ms": round(elapsed_ms, 1), "status": "ok"})
            except Exception as e:
                if step.tool == "answer.compose" or not continue_on_error:
                    raise
